...
const insertImage = async (img) => {
  const query = `
    INSERT INTO img_objects (img_name, pHash, id_object)
    VALUES ($1, $2, $3)
    RETURNING _id_img
  `;
...
const getAllImages = async () => {
  const result = await db.query('SELECT * FROM img_objects');
  return result.rows;
};

// 📥 Récupérer les images liées à un objet
const getImagesByObjectId = async (objectId) => {
  const query = `SELECT * FROM img_objects WHERE id_object = $1`;
...
const deleteImage = async (id) => {
  const query = `DELETE FROM img_objects WHERE _id_img = $1`;
