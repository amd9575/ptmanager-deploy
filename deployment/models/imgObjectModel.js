...
const insertImage = async (img) => {
  const query = `
    INSERT INTO image_object (img_name, pHash, id_object)
    VALUES ($1, $2, $3)
    RETURNING _id_img
  `;
...
const getImagesByObjectId = async (objectId) => {
  const query = `SELECT * FROM image_object WHERE id_object = $1`;
...
const deleteImage = async (id) => {
  const query = `DELETE FROM image_object WHERE _id_img = $1`;
